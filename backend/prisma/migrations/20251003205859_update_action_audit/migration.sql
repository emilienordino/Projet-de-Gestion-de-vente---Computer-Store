/*
  ...
  - The values [INSERT] on the enum `audit_action` will be removed. If these variants are still used in the database, this will fail.
...
-- AlterTable
ALTER TABLE `audit` MODIFY `action` ENUM('CREATE', 'UPDATE', 'DELETE', 'LOGIN', 'LOGOUT', 'LOGIN_FAILED', 'RESTORE', 'CHANGE_STATUS', 'GENERATE_INVOICE', 'ADD_PAYMENT', 'REFUND', 'CANCEL', 'APPLY_DISCOUNT', 'STOCK_UPDATE') NOT NULL;
